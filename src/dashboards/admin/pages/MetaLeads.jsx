...
export default function MetaLeads() {
  ...
  return (
    <div className="p-4 md:p-6 space-y-6 max-w-[1400px] mx-auto animate-fadeIn min-h-screen">
      ...
      {/* Toast */}
      {toast && (
        <div className={`flex items-center gap-3 p-4 rounded-xl border text-sm animate-fadeIn ${
          toast.type === 'success'
              ...
              {loading ? (
                ...
                <tr><td colSpan="8" className="px-6 py-16 text-center">
                  <div className="flex flex-col items-center gap-3 text-gray-400">
                    ...
                    <span className="text-sm font-medium">No meta leads found.</span>
                  </div>
                </td></tr>
              ) : leads.map(lead => (
                <tr key={String(lead._id)} className="hover:bg-blue-50/20 dark:hover:bg-gray-800/30 transition-colors">
                  ...
                  <td className="px-5 py-4">
                    ...
                    <div className="text-[10px] text-gray-400 mt-0.5">{lead.number || '—'}</div>
                  </td>
                  <td className="px-5 py-4">
